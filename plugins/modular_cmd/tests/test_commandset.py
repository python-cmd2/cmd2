...
from cmd2_modular_cmds import ModularCommandsMixin, CommandSet, register_command, with_default_category, command_definition
...
@with_default_category('Command Set')
class CommandSetA(CommandSet):
...
class WithModularCommands(ModularCommandsMixin, cmd2.Cmd):
    """Class for testing custom help_* methods which override docstring help."""
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)


@with_default_category('Command Set B')
class CommandSetB(CommandSet):
...
def command_sets_app():
    app = WithModularCommands()
    return app


@pytest.fixture()
def command_sets_manual():
    app = WithModularCommands(auto_load_commands=False)
    return app
...
def test_custom_construct_commandsets():
    ...
    app = WithModularCommands(command_sets=[command_set])
...
def test_load_commands(command_sets_manual):
    ...
    with pytest.raises(KeyError):
        assert register_command(do_unbound)
...
def test_partial_with_passthru():
    ...
    my_partial = command_definition._partial_passthru(test_func, 1)
